...
require the database instance reboot.
...
This will let Teleport Database Service to generate an IAM auth token when
...
Asign it the "Service Account Token Creator" role:
...
```sh
$ tctl --config=/path/to/teleport.yaml users add --roles=editor,access,db testuser
```
...
$ psql "service=teleport.example.com-cloudsql user=teleport@<project-id>.iam dbname=metrics"
```
