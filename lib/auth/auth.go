...
func (a *Server) ExtendWebSession(req WebSessionReq, identity tlsca.Identity) (services.WebSession, error) {
	...
	if req.Switchback {
		...
		fmt.Println("------ BEFORE switching back. roles: ", roles, ", default expiry: ", expiresAt)

		roles = user.GetRoles()
		expiresAt = prevSession.GetDefaultExpiryTime()

		fmt.Println("------ AFTER switching back. roles: ", roles, ", default expiry: ", expiresAt)
	}
	...
	// Preserve the default expiry time derived from first session creation.
...
func (a *Server) NewWebSession(req types.NewWebSessionRequest) (services.WebSession, error) {
	fmt.Println("------------------ NewWebSession, isFirstSession?: ", req.IsFirstSession)
	user, err := a.GetUser(req.User, false)
	if err != nil {
		return nil, trace.Wrap(err)
	}
