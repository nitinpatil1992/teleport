...
func (s *APIServer) createWebSession(auth ClientI, w http.ResponseWriter, r *http.Request, p httprouter.Params, version string) (interface{}, error) {
	...
	// DELETE IN ---: proxy v--sends request with no user field.
	// And since proxy v6.2, request will come with user field set, so grabbing user
	// by param is no longer required.
	user := p.ByName("user")
	req.User = user
